...
class TBXExportView(APIView):
    def get(self, request, format=None):
        return Response("TBX export is not implemented yet", status=HTTP_501_NOT_IMPLEMENTED)
...
def import_definitions(term_entry, concept):
    ...
    if definition:
        ...
        concept.save()
        print("imported definition for concept: {}: {}".format(concept, definition))
...
def import_term(tig, lang, concept):
    ...
    if not pos:
        logger.warning(u"Could not import term '{}': no partOfSpeech data found".format(term))
        return
    pos = pos.lower()
    ...
    if not lemma_form:
        ...
        if not lemma_form:
            lemma_form = Form(name='term', lexicalClass=lex_class, preference=1000)
            lemma_form.save()
            print("created temporary form: term for lexical class: {}".format(pos))
    # make an attempt to check if the lexeme already exists (based on its lemma)
    lexeme = get_or_none(Lexeme, language=lang, lexicalClass=lex_class, lemma=term)
    if not lexeme:
...
# Blocks of text in the XML have newlines and tabs for alignment - clean these up
